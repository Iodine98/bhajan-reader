...
secrets (`TAILSCALE_BACKEND_URL`, `GATEWAY_API_KEY`, and `GATEWAY_ORIGIN`,
e.g. `https://bhajan-server.vital-dev.com`) — it's never committed, and
`.htaccess` denies HTTP access to it on the server.
